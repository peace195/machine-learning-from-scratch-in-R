...
% Load MNIST Train
fprintf('Load MNIST Train\n');
images = csvread('../train.csv', 1, 0);
labels = images(:, 1);
sizeImages = size(images);
images = images(:, 2:sizeImages(2));
...
[~,cost,preds]=cnnCost(opttheta,testImages,[],numClasses,...
                filterDim,numFilters,poolDim,true);

preds(preds == 10) = 0;
sizeTest = size(testImages);
ret = [1:sizeTest(3); preds'];
ret = ret';
csvwriteh('submisson.csv',ret, {'ImageId', 'Label'})
