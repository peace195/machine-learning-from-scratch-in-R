...
#train <- (train - matrix(sum(train)/(dim(train)[1] * dim(train)[2]), dim(train)[1], dim(train)[2]))/255

#test <- (test - matrix(sum(test)/(dim(test)[1] * dim(test)[2]), dim(test)[1], dim(test)[2]))/255

train <- as.matrix(cbind(1, train))
